...
Для навигации и роутинга в приложении используется пакет компонентов react-router-dom.
За конфигурациию хранилища, создание actions и reducers отвечает Redux-toolkit.
В директории `./store` хранилище для удобства разбито на два согласно предметной области.
В файле `./services/api.ts` описана конфигурация API для взаимодействия с удаленным сервером с помощью пакета axios.
В файле `./store/index.ts` с помощью middleware thunk передается экземпляр axios для конфигурации хранилища.
Для отрисовки меток на карте используется сервис leaflet.
